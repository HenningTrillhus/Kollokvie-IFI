# Teknisk dokumentasjon: oppsett, database og drift

Dette er detaljene for deg som drifter eller videreutvikler Kollokvie@IFI: hvordan prosjektet settes opp, databasemigreringene, e-post, personvern og sikkerhet. Oversikten over hva appen er ligger i [README.md](../README.md).
...
- **Profiler og følgere.** Følgeforespørsler må godkjennes. Følger- og følgerlister er
  ...
  blant 85 ferdige ikoner. Uten bilde vises forbokstaven på en aksentfarge du velger.
...
- **Kalender** med måneds- og semestervisning. Semestrene følger UiOs datoer (høst og vår, med kilde-lenke),
  ...
  hvilke emner som vises og hvilken farge hvert emne har. Obliger og annet kan markeres som ferdige (med konfetti), og havner bakerst. Dager med noe på er farget, og typen (eksamen, oblig, annet, gruppe) står med små bokstaver i dagen. En eksamen eller oblig uten tittel får bare navnet «Eksamen» eller «Oblig». På PC har semestervisningen en lang, tynn oversikt til venstre og bare obliger og eksamener (som kan krysses av) til høyre. Kollokviegruppene dine står på riktig dato.
...
| Fil | Innhold |
| --- | --- |
| `0049` | `user_deadline_completions`: huker av automatisk lagt-inn obliger, per bruker |
...
| **Filopplasting** | Det eneste som kan lastes opp er profilbilder, og de sjekkes i to trinn. I nettleseren: maks 8 MB inn, den *ekte* typen avgjøres av filens første bytes (ikke filnavn eller oppgitt type), bare JPEG, PNG og WebP (ikke SVG, som kan bære skript, og ikke GIF), maks 24 megapiksler, alltid skalert til 256×256 og komprimert til under 100 KB (metadata som posisjon fjernes). På serveren (`POST /api/avatar`, `src/lib/avatar-server.ts`): samme sjekker på nytt, grense på 256 KB per opplasting og 8 per minutt per bruker, kun fra egen side (Origin-sjekk), og bildet dekodes og **re-enkodes** med sharp, så bare rene piksler lagres. Bøtta tar til slutt bare imot `image/jpeg` under 100 KB, og bare filen `<din-id>/avatar.jpg` (migrering 0031 og 0040). |
...
| **Admin og API-ruter** | Appen har ingen admin-sider. Den har én egen API-rute, `POST /api/avatar` (profilbilder, se over), som krever innlogging, sjekker Origin og har grenser. I tillegg `/.well-known/security.txt`. Alle sider bak innlogging sjekkes i `src/proxy.ts` (JWT verifiseres mot Supabase sin offentlige nøkkel) og på nytt i selve siden. Åpne sider matches nøyaktig, så `/vilkarfoo` ikke blir åpen ved en feil. Alle andre data går via Supabase-API-et, som styres av Row Level Security. |
...
| **Rate limiting og kvoter** | Tre lag, alle satt høyt så vanlig bruk aldri merker dem. (1) Databasen: hver bruker har grenser per minutt og time på å opprette hendelser, grupper, følgere, medlemskap, invitasjoner, emner og så videre (migrering 0037), og kvoter på totalt antall (0033). (2) Webserveren: `src/proxy.ts` begrenser forespørsler per IP-adresse (6000 per minutt) og per innloggingsøkt (1200 per minutt), og stenger en adresse ute i 5 minutter hvis den sender mer enn 1,5 ganger taket. Tellerne ligger i serverinstansens minne, så hver instans beskytter seg selv. (3) Innlogging, e-postkoder og passord-reset begrenses av Supabase Auth (se under). Når noe avvises, viser appen «Rolig, du gjør ting litt for fort». |
...
- **Supabase → Authentication → Rate Limits:** behold lave grenser på e-post (koder), innlogging og verifisering per IP.
- **Vercel → Firewall:** legg til en regel som begrenser forespørsler per IP-adresse globalt (for eksempel 3000 per minutt til `/*`), og slå på «Bot Protection». Serverens egen grense gjelder bare innen én instans; dette er den globale.
- Studenter deler ofte nettverksadresse på campus, så ikke sett grenser per IP lavere enn dette.
...
- **Lesing av data (SELECT) har ingen egen hastighetsgrense** i databasen, bare tidsavbrudd på spørringer og maks antall rader per svar. Skriving er begrenset (0037). Vercel Firewall kan legge på en global IP-grense.
